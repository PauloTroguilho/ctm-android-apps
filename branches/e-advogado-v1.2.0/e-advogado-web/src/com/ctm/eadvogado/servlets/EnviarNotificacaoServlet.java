...
public class EnviarNotificacaoServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (usuario != null) {
			...
			log(String.format("Enviando notificação para o usuário: %s, params: %s", pEmail, paramsMap));
			try {
				usuarioNegocio.enviarNotificacao(usuario, paramsMap);
			} catch (Exception e) {
				log(String.format("Falha ao enviar notificação para o usuário: %s ", pEmail), e);
			}
		} else {
